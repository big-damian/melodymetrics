...
class DatasetFileNotFoundException(Exception):
    """
    Exception raised when a dataset file is not found.
    ...
    def __init__(self, message="default"):
        """
        Initializes the DatasetFileNotFoundException with a message.

        Parameters
        ----------
        message : str, optional
            The error message to display. Defaults to "No dataset file found in directories.
            Please download the dataset first." if "default" is passed.
        """
        if message == "default":
            message = "No dataset file found in directories. Please download the dataset first."
        super().__init__(message)
